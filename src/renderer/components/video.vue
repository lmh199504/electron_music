            ...
            <el-row :gutter="20">
                <el-col :xs="6" :sm="6" :md="6" :lg="4" v-for="item in list" :key="item.dissid">
                    <div class="item-container">
                        <div class="item" :style="`background: url(${item.picurl}) 0 0 no-repeat;background-size:cover;`" @click="toDetail(item)">
                            <div class="mask">
                            </div>
                            <div class="playIcon">
                        ...
                        <div class="item-foot">
                            <p class="dissname">{{item.mvtitle}}</p>
                            <p class="creator">{{item.singername}}</p>
                        </div>
                    </div>
                </el-col>
            </el-row>
...
export default {
    ...
    created() {
        this.$Api.getMvByTag().then(res => {
            ...
            this.list = JSON.parse(res).data.mvlist;
        })
    },
    filters: {
        numberFormat(num) {
