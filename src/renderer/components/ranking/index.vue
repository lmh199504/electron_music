<template>
	<div class="discoDetail ">
		<div class="top mycontainer">
			...
			<div class="top-right">
				<h1 style="margin-top: 5px;">{{topTitle}}</h1>
		...
		<div class="content">
			<div class="mycontainer">
				<musiclist v-bind:list="listArr"></musiclist>
			</div>
		</div>
	</div>
</template>
...
<script>
	...
	export default{
	  ...
	  created () {
	    ...
	    this.topTitle = this.$route.query.item.topTitle
	    this.coverImg = this.$route.query.item.picUrl
	    this.$Api.getTopDetail(this.$route.query.item.id).then(res => {
	      // console.log(res.songlist)
	      this.updatatime = res.update_time
	      for (let i = 0; i < res.songlist.length; i++) {
	        let list = res.songlist[i]
			// console.log(list.albumid)
	        this.listArr.push({
	          img: 'http://imgcache.qq.com/music/photo/album_300/17/300_albumpic_' + list.data.albumid + '_0.jpg',
	          singer: list.data.singer[0].name,
	          songid: list.data.songid,
	          songmid: list.data.songmid,
	          songname: list.data.songname,
	          albumname: list.data.albumname,
	          interval: list.data.interval,
	          albumdesc: list.data.albumdesc
	        })
	      }
	    }).catch(() => {
	      this.$router.push({
